...
def test(model, loader, do_map=True):
    tmp=[]
    with torch.no_grad():
        mean_correct = []
        if do_map:
        ...
        if do_map:
            map, aps,r,p = mean_average_precision(outputs, targets)
            return np.mean(mean_correct), map, aps,r,p
...
def mean_average_precision(output, target):
    ...
    with torch.no_grad():
        ...
        for n in range(6):
            ...
            aps += [ap]
    return np.mean(aps), aps,recs/6,precs/6

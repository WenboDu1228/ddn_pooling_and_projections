...
class AbstractLearnableDeclarativeNode(AbstractNode):
    """
    ...
    def gradient(self, x, y=None,theta=None, ctx=None):
        """
        ...
        if y is None:
            y, ctx = self.solve(x)
        assert self._check_optimality_cond(x, y, theta)

        return -1.0 * sci.linalg.solve(self.fYY(x, y, theta), self.fXY(x, y, theta), assume_a='pos'), -1.0 * sci.linalg.solve(self.fYY(x, y, theta), self.fthetaY(x, y, theta), assume_a='pos')

    def _check_optimality_cond(self, x, y,theta, ctx=None):
        """Checks that the problem's first-order optimality condition is satisfied."""
        return (abs(self.fY(x, y, theta)) <= self.eps).all()
...
class NonUniqueLearnableDeclarativeNode(AbstractLearnableDeclarativeNode):
    """
    A general deep declarative node having non-unique solutions so that the pseudo-inverse is required
    in computing the gradient.
    """
    ...
    def gradient(self, x, y=None,theta=None, ctx=None):
        ...
        if y is None:
            y, ctx = self.solve(x)
        assert self._check_optimality_cond(x, y,theta, ctx), abs(self.fY(x, y,theta))

        return -1.0 * np.linalg.pinv(self.fYY(x, y, theta)).dot(self.fXY(x, y, theta)),-1.0 * np.linalg.pinv(self.fYY(x, y, theta)).dot(self.fthetaY(x, y, theta))
    
class EqConstLearnableDeclarativeNode(AbstractDeclarativeNode):
    """
    A general deep declarative node defined by a parameterized optimization problem with single (non-linear)
    equality constraint of the form
        minimize (over y) f(x, y)

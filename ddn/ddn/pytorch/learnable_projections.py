...
class L1Sphere(Simplex):
    ...
    def gradient(grad_output, output, input, is_outside = None):
        ...
        # 3. Unflatten:
        grad_input = grad_input.reshape(output_size)
        Dr= torch.sum(grad_output*(DYh/(DYh * DYh).sum(-1, keepdim=True)),dim=-1)
        return grad_input,Dr
...
class L1Ball(L1Sphere):
    ...
    def gradient(grad_output, output, input, is_outside):
        ...
        grad_input,Dr = L1Sphere.gradient(grad_output, output, input, is_outside)
        ...
        # outside: torch.Size([128, 1])
        #print('Dr',Dr,Dr.shape)
        #print('gradoutput',grad_output,grad_output.shape)
        #Dr torch.Size([128])
        #print('is_outside',is_outside)
        #print(Dr)
        Dr=torch.where(is_outside.squeeze(-1), Dr, torch.zeros_like(Dr))
        #print('Dr final',Dr,Dr.shape)
        return grad_input,Dr
...
class L2Sphere():
    ...
    def gradient(grad_output, output, input, is_outside = None):
        ...
        # 3. Unflatten:
        grad_input = grad_input.reshape(output_size)
        #print(input,input_norm)
        #Dr=(input.div(input_norm)
        #print('Dr:',Dr.shape,grad_output.shape)
        Dr=(input.div(input_norm)*grad_output).sum(-1)
        return grad_input,Dr
...
class L2Ball(L2Sphere):
    ...
    def gradient(grad_output, output, input, is_outside):
        # Compute vector-Jacobian product (grad_output * Dy(x))
        # 1. Compute constrained gradient
        grad_input,Dr = L2Sphere.gradient(grad_output, output, input, is_outside)
        # 2. If input was already inside ball (or on surface), use unconstrained gradient instead
        grad_input = torch.where(is_outside, grad_input, grad_output)
        #print(is_outside,Dr)
        Dr=torch.where(is_outside.squeeze(-1), Dr, torch.zeros_like(Dr))
...
class LInfSphere():
    ...
    def gradient(grad_output, output, input, is_outside = None):
        ...
        # 3. Unflatten:
        grad_input = grad_input.reshape(output_size)
        nominor=(hY * hY).sum(-1, keepdim=True).expand_as(hY)
        Dr=torch.where(nominor.abs()<= 1e-9,torch.zeros_like(nominor),grad_output*(hY.div(nominor)))
        Dr=Dr.sum(dim=-1)
        Dr=torch.where(Dr>=0, torch.where(Dr>0,torch.ones_like(Dr),torch.zeros_like(Dr)),-torch.ones_like(Dr))
        return grad_input,Dr
...
class LInfBall(LInfSphere):
    ...
    def gradient(grad_output, output, input, is_outside):
        # Compute vector-Jacobian product (grad_output * Dy(x))
        # 1. Compute constrained gradient
        grad_input,Dr = LInfSphere.gradient(grad_output, output, input, is_outside)
        # 2. If input was already inside ball (or on surface), use unconstrained gradient instead
        grad_input = torch.where(is_outside, grad_input, grad_output)
        Dr=torch.where(is_outside.squeeze(-1), Dr, torch.zeros_like(Dr))
        return grad_input,Dr
...
class EuclideanProjectionFn(torch.autograd.Function):
    ...
    def forward(ctx, input, method, radius):
        ...
        #print('outside:',is_outside.shape)
        ctx.method = method
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_input_x, grad_input_r= ctx.method.gradient(grad_output, output, input, is_outside)
        return grad_input_x, None, grad_input_r

class EuclideanProjection(torch.nn.Module):
    def __init__(self, method, radius = 1.0,train_radius=False):
        super(EuclideanProjection, self).__init__()
        self.method = method
        print('init radius',radius)
        radius=util.inv_affine_softplus(radius,lo=1e-5,ref=250.0)
        if train_radius==False:
            self.register_buffer('radius', torch.tensor([radius]))
        else:
            print("adaptive radius",radius)
            self.radius=torch.nn.Parameter(torch.tensor([radius],requires_grad=True,device='cuda'))

    def forward(self, input):
        radius=util.affine_softplus(self.radius,lo=1e-5,ref=250.0)
        #print('r',radius)

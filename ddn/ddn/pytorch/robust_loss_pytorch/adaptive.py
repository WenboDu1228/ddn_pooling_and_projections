...
from ddn.pytorch.robust_loss_pytorch import distribution
from ddn.pytorch.robust_loss_pytorch import util
from ddn.pytorch.robust_loss_pytorch import wavelet
...
class AdaptiveLossFunction(nn.Module):
  ...
  def __init__(self,
    ...
    if alpha_lo == alpha_hi:
      ...
      self.alpha = lambda latent_alpha: self.fixed_alpha
    else:
      ...
      self.alpha= lambda latent_alpha: util.affine_sigmoid(
          latent_alpha, lo=alpha_lo, hi=alpha_hi)
    ...
    if scale_lo == scale_init:
      ...
      self.scale= lambda latent_scale: self.fixed_scale
    else:
      ...
      self.scale = lambda latent_scale: util.affine_softplus(
          latent_scale, lo=scale_lo, ref=scale_init)
 # def lossfun(self, x, **kwargs):
  def lossfun(self, x, latent_alpha,latent_scale, **kwargs):
    ...
    return self.distribution.nllfun(x,self.alpha(latent_alpha),self.scale(latent_scale), **kwargs)

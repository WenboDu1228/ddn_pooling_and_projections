...
import numpy as np
import torch
from ddn.pytorch.robust_loss_pytorch import cubic_spline
from ddn.pytorch.robust_loss_pytorch import general
...
class Distribution():
  ...
  def nllfun(self, x, alpha, scale):
    ...
    assert (alpha >= 0).all()
    assert (scale >= 0).all() , print('the scale:',scale)
    float_dtype = x.dtype
    ...
    #print('log',self.log_base_partition_function(alpha))
    #log_partition = torch.log(scale)+torch.tensor([[0.1555]])
    nll = loss + log_partition
    # print(nll.sum(dim=(-2,-1)))
    #nll=loss
    return nll

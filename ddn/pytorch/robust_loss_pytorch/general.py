...
def lossfun(x, alpha, scale, approximate=False, epsilon=1e-6):
  ...
  if approximate:
    ...
    #print('alpha value:',alpha==-float('inf'))
  ...
  #loss = scale**2 * (torch.sqrt(1.0 + torch.pow(x, 2) / (scale**2)) - 1.0)
  #loss = torch.where(x <= 1.0, 0.5 * torch.pow(x, 2), 0.5 * 1.0 * 1.0 * torch.ones_like(x))
  return loss
  #return loss_otherwise
